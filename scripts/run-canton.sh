...
if [[ ${1} == "--all" ]] ; then
	...
	for i in *KW*.pdf ; do
		for canton in ${cantons[*]} ; do
			python ../../scripts/parse_canton_data.py ${canton} "${i}" >> ../../${out_dir}/bag_data_${canton}.csv
		done
	done
	...
	for canton in ${cantons[*]} ; do
		python ../scripts/parse_canton_data.py ${canton} BAG_COVID-19_Woechentliche_Lage.pdf >> ../${out_dir}/bag_data_${canton}.csv
	done
fi
